...
const About = ({ isDarkMode }) => {
  ...
  return (
    <div id="about" ref={sectionRef} className="relative w-full px-4 py-16 mt-20 sm:px-6 md:px-12 lg:pt-32">
      <h4 className={`mb-2 text-lg text-center font-Ovo text-gray-600 dark:text-[#FFFFFF] transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>Introduction</h4>
      <h2 className={`text-4xl text-center sm:text-5xl font-Ovo text-[#1a202c] dark:text-[#01C38D] transition-all duration-700 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>About me</h2>
      ...
      <div className="flex flex-col items-center max-w-6xl gap-12 mx-auto my-16 lg:flex-row lg:gap-20">
        ...
        <div className={`flex-1 text-center lg:text-left transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20'}`}>
          <p className="max-w-2xl mx-auto mb-10 text-gray-600 lg:mx-0 font-Ovo dark:text-[#FFFFFF] leading-relaxed">
          ...
          <ul className="grid max-w-2xl grid-cols-1 gap-6 mx-auto sm:grid-cols-3 lg:mx-0">
            {infoList.map(({ icon, iconDark, title, description }, i) => (
              ...
              >
                ...
                <h3 className="my-4 font-semibold text-[#2d3748] dark:text-[#01C38D]">
                  {title}
                </h3>
