...
const Works = ({ isDarkMode }) => {
  ...
  return (
    ...
    >
      <h4 className={`mb-2 text-lg text-center font-Ovo text-gray-600 dark:text-[#FFFFFF] transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        My portfolio
      </h4>
      <h2 className={`text-4xl text-center sm:text-5xl font-Ovo text-[#1a202c] dark:text-[#01C38D] transition-all duration-700 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        My latest works
      </h2>
      ...
      <div className="grid gap-6 my-10 grid-cols sm:grid-cols-2 lg:grid-cols-4">
        {workData.map((project, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-4 transition-all duration-300 opacity-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent group-hover:opacity-100 translate-y-4 group-hover:translate-y-0">
              <h2 className="text-lg font-semibold text-white transform transition-transform duration-300 translate-y-4 group-hover:translate-y-0">
                {project.title}
              </h2>
              <p className="text-sm text-gray-300 transform transition-all duration-300 delay-75 translate-y-4 group-hover:translate-y-0">{project.description}</p>
            </div>

            <div className="absolute bottom-4 right-4 transition-all duration-200 opacity-70 group-hover:opacity-100 group-hover:scale-110 group-hover:rotate-12">
              <Image
